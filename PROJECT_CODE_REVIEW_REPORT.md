...
| 工作流 | 文件 | 触发条件 | 说明 |
|--------|------|----------|------|
| CI/CD Pipeline | `ci-cd.yml` | Push/PR | 单元测试、构建、安全扫描 |
| Integration Tests | `integration-tests.yml` | CI/CD成功后 | 集成测试（数据库、服务协作） |
| E2E Tests | `e2e-tests.yml` | 集成测试成功后 | 端到端测试（Playwright） |

**测试工作流执行顺序**：
```
Push/PR → CI/CD (单元测试) → Integration Tests → E2E Tests
   ↓           ~2-3分钟          ~5-10分钟         ~15-20分钟
快速反馈      代码质量验证       模块协作验证       完整验收测试
```
...
**报告更新时间**：2026-03-19 20:00  
**验证状态**：✅ 已通过实际测试验证  
**CI状态**：✅ GitHub Actions 全部通过  
**E2E测试**：✅ Playwright测试工作流已配置  
**负责人**：开发团队全体
