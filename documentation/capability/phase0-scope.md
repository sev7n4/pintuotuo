...
| 部署流水线**非致命**跑一遍默认（低成本）探测 | 全量 `-billable` 探测作为 CI 必过门禁（默认否） |
...
|------|----------------|--------------------------------------|
| **部署后流水线 / 日常巡检** | 不加 `-billable` | 约 **3**（embeddings + moderations + responses；若 `-skip-embeddings` 则 2） |
...
- **`.github/workflows/deploy-tencent.yml`**：在容器启动后**非致命**执行默认（无 `-billable`）探测，并将 CSV 尾部打印到日志，便于在 GitHub Actions 中查看摘要。
- **全量计费探测**不在 CI 中默认开启；由运维在服务器上手工执行并保存 CSV。

## 5. Phase 0 完成判据（DoD 补充）

- [ ] 生产镜像中存在 `/app/capability-probe` 且 `docker exec … test -x` 通过。
- [ ] 至少一次部署日志或手工归档的 CSV 中，对**每个活跃 BYOK provider** 有 `get_models` 行；对承诺的 OpenAI 系 provider 有 `post_*` 行（含 `skipped` 或 HTTP 码）。
