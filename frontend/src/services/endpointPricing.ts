...
import type {
  EndpointPricing,
  EndpointPricingCreateRequest,
  EndpointPricingUpdateRequest,
} from '@/types/sku';
...
export const endpointPricingService = {
  getList: (params?: {
    ...
    api.get<{ total: number; page: number; per_page: number; data: EndpointPricing[] }>(
      '/admin/endpoint-pricing',
      { params }
    ),
  ...
  delete: (id: number) => api.delete(`/admin/endpoint-pricing/${id}`),
};
