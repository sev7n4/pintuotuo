...
export const merchantSkuService = {
  async getMerchantSKUs(status?: string): Promise<MerchantSKUDetail[]> {
    ...
    const response = await api.get<APIResponse<MerchantSKUDetail[]>>(
      `/merchants/skus?${params.toString()}`
    );
  ...
  async getAvailableSKUs(provider?: string, type?: string): Promise<AvailableSKU[]> {
    ...
    const response = await api.get<APIResponse<AvailableSKU[]>>(
      `/merchants/skus/available?${params.toString()}`
    );
    return response.data.data || [];
  },
