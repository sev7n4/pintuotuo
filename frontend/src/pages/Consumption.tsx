...
const Consumption: React.FC = () => {
  ...
  const exportData = () => {
    const csv = [
      ['请求ID', 'Provider', 'Model', '输入Tokens', '输出Tokens', '费用($)', '延迟(ms)', '状态码', '时间'].join(','),
      ...records.map(r => [
        r.request_id,
        r.provider,
        r.model,
        ...
        {selectedRecord && (
          <Descriptions column={2} bordered size="small">
            ...
            <Descriptions.Item label="Provider">
              <Tag color={getProviderColor(selectedRecord.provider)}>{selectedRecord.provider.toUpperCase()}</Tag>
            </Descriptions.Item>
            ...
            <Descriptions.Item label="输入Tokens">{selectedRecord.input_tokens.toLocaleString()}</Descriptions.Item>
            <Descriptions.Item label="输出Tokens">{selectedRecord.output_tokens.toLocaleString()}</Descriptions.Item>
            <Descriptions.Item label="费用">
              <span style={{ color: '#f5222d', fontWeight: 'bold' }}>${selectedRecord.cost.toFixed(6)}</span>
            </Descriptions.Item>
