...
const mockCoupons: Coupon[] = [
  { id: 1, name: '新用户专享券', type: 'cash', value: 10, min_purchase: 50, used_count: 156, total_count: 500, status: 'active', expire_at: '2026-04-30' },
  { id: 2, name: '满100减20', type: 'cash', value: 20, min_purchase: 100, used_count: 89, total_count: 200, status: 'active', expire_at: '2026-03-31' },
  { id: 3, name: '全场9折券', type: 'discount', value: 10, min_purchase: 0, used_count: 234, total_count: 300, status: 'active', expire_at: '2026-04-15' },
]

const mockFlashSales: FlashSale[] = [
  { id: 1, product_id: 101, product_name: 'GLM-5 Token包', original_price: 100, flash_price: 59, stock: 100, sold: 78, start_time: '2026-03-24 10:00', end_time: '2026-03-24 12:00', status: 'active' },
  { id: 2, product_id: 102, product_name: 'K2.5 Token包', original_price: 150, flash_price: 99, stock: 50, sold: 50, start_time: '2026-03-24 14:00', end_time: '2026-03-24 16:00', status: 'ended' },
]
...
export const MerchantMarketing: React.FC = () => {
  ...
  const couponColumns = [
    ...
    { 
      ...
      dataIndex: 'type', 
      key: 'type',
      render: (type: string) => type === 'cash' ? <Tag color="blue">满减券</Tag> : <Tag color="green">折扣券</Tag>
    },
    { 
      title: '优惠内容', 
      key: 'value',
      render: (_: unknown, record: Coupon) => record.type === 'cash' ? `减¥${record.value}` : `${100 - record.value}折`
    },
    { title: '使用门槛', dataIndex: 'min_purchase', key: 'min_purchase', render: (v: number) => v > 0 ? `满¥${v}` : '无门槛' },
    { title: '已用/总量', key: 'usage', render: (_: unknown, record: Coupon) => `${record.used_count}/${record.total_count}` },
    { title: '有效期', dataIndex: 'expire_at', key: 'expire_at' },
    { 
      title: '状态', 
      dataIndex: 'status', 
      key: 'status',
      render: (status: string) => status === 'active' ? <Tag color="success">生效中</Tag> : <Tag color="default">已停用</Tag>
    },
    {
      title: '操作',
      key: 'action',
  ...
  const flashSaleColumns = [
    ...
    { 
      ...
      render: (_: unknown, record: FlashSale) => (
        <Space>
          <Text delete type="secondary">¥{record.original_price}</Text>
          <Text type="danger" strong>¥{record.flash_price}</Text>
        </Space>
      )
    },
    ...
    <div className={styles.container}>
      ...
      <Tabs defaultActiveKey="coupons">
        <TabPane tab="优惠券管理" key="coupons">
          <Card>
            <div style={{ marginBottom: 16 }}>
              <Button type="primary" icon={<PlusOutlined />} onClick={() => setCreateCouponModalVisible(true)}>
                创建优惠券
              </Button>
            </div>
        ...
        <TabPane tab="限时秒杀" key="flash-sale">
          <Card>
            <div style={{ marginBottom: 16 }}>
              <Button type="primary" icon={<ThunderboltOutlined />} onClick={() => setCreateFlashSaleModalVisible(true)}>
                创建秒杀活动
              </Button>
            </div>
        ...
        <TabPane tab="平台活动" key="platform">
          <Card title="平台大促活动">
            <List
              ...
              renderItem={(item) => (
                <List.Item
                  actions={[<Button type="link" key="join">立即报名</Button>]}
                >
                  <List.Item.Meta
                    title={item.name}
