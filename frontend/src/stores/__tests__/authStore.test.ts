...
describe('AuthStore', () => {
  ...
  describe('register', () => {
    ...
    it('should handle registration error', async () => {
      ...
      const store = useAuthStore.getState()
      await expect(store.register('existing@example.com', 'User', 'password')).rejects.toThrow(errorMessage)
  ...
  describe('logout', () => {
    it('should logout successfully', async () => {
      ...
      // 测试登出
      mockedAuthService.logout.mockResolvedValueOnce({ data: { code: 0, message: 'success' } } as any)
      await store.logout()
